...
/// Handles `grinta://fitbit/callback?success=1&playerId=...` after OAuth.
class FitbitDeepLinkService {
  FitbitDeepLinkService._();

  static final FitbitDeepLinkService instance = FitbitDeepLinkService._();

  final AppLinks _appLinks = AppLinks();
  StreamSubscription<Uri>? _linkSubscription;
  bool _initialized = false;

  final ValueNotifier<FitbitOAuthCallbackResult?> lastCallbackResult =
      ValueNotifier(null);
  ...
  void _handleUri(Uri uri, {required String source}) {
    ...
    final result = FitbitOAuthCallbackResult(
      success: success,
      error: error,
      playerId: playerId,
    );
    ...
    if (success) {
      AppSnackbar.show(
        context,
        context.l10n.fitbitConnectSuccess,
        isError: false,
      );
    } else {
      AppSnackbar.show(
        context,
        context.l10n.fitbitConnectFailed,
        isError: true,
      );
    }
  }

  static bool _isFitbitCallback(Uri uri) {
    if (uri.scheme != 'grinta') return false;
    if (uri.host == kFitbitOAuthDeepLinkHost) return true;
    if (uri.path == kFitbitOAuthDeepLinkPath ||
        uri.path == '$kFitbitOAuthDeepLinkPath/') {
      return true;
    }
    return false;
  }
  ...
  void _log(String message) {
    ...
    debugPrint('[FitbitDeepLink] $message');
  }
}

class FitbitOAuthCallbackResult {
  const FitbitOAuthCallbackResult({
