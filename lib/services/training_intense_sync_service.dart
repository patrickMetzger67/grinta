...
///
/// Prefers full-time Temps forts, else [Match.timestamp] + duration + 15'
/// half-time break (no Temps forts required). Never uses dateCh/timeCh.
...
///
/// Prefers field-linked [Match.timestamp] so a Temps forts « début » tapped
/// after the match (wall-clock ≠ real kick-off) cannot collapse the window to
/// `start == stop` and return 0 GNSS samples. Never uses dateCh/timeCh.
///
...
/// [Match.timestamp] (`[T, T+45]` then `[T+60, T+105]` for a 90' match). The
...
/// [SensorAnalysisService] via [GpsDistanceSmoother] (median windows + speed gate).
