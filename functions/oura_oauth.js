...
/** Scopes for Phase 1 connect + workout import (sleep/readiness sync = Phase 2). */
const OURA_SCOPES = [
  'email',
  'personal',
  'daily',
...
async function fetchOuraPersonalInfo(accessToken) {
  ...
  if (!response.ok) {
    ...
    console.error('Oura personal_info fetch failed', response.status, body);
    return null;
  }
  ...
  return ouraUserId != null ? String(ouraUserId) : null;
}
...
async function persistOuraConnection(db, pending, tokenResponse) {
  ...
  const ouraUserId = await fetchOuraPersonalInfo(accessToken);
  ...
  const grantedScopes = (tokenResponse?.scope ?? OURA_SCOPES)
    .toString()
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  ...
  await db.runTransaction(async (transaction) => {
    transaction.set(
      integrationRef,
      {
        ...
        status: 'connected',
        scopes: grantedScopes.length ? grantedScopes : OURA_SCOPES.split(' '),
...
async function revokeOuraToken(accessToken) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Oura token revoke failed', response.status);
    }
  } catch (err) {
    console.warn('Oura token revoke error', err);
  }
}
 ...
 *
 * Request: { playerId, initiatedBy: "coach"|"player", ouraAccountHint }
    ...
    async (request) => {
      ...
      for (const ownerUid of ownerUids) {
        const snap = await db
          ...
          .get();
        const token = (snap.data()?.tokens?.accessToken ?? '').toString();
        if (token) accessTokens.push(token);
      }
